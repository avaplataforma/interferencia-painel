...
$organizationAnnouncements = new \Interferencia\Modules\Organization\OrganizationAnnouncements($database);
$organizationPortalSettings = new \Interferencia\Modules\Organization\OrganizationPortalSettings($database);
$organizationMaterials = new \Interferencia\Modules\Organization\OrganizationMaterials($database);
$organizationSatisfaction = new \Interferencia\Modules\Organization\OrganizationSatisfaction($database);
$materialFiles = new MediaStorage($rootPath . '/storage/materials', $spacesStorage, $storageScope, $organizationId, 'Materiais');
...
$centralEmailService = new CentralEmailService($centralEmails,'https://'.($centralHost??$config->string('app.central_host')));
...
$avaCatalogPublisher=new \Interferencia\Modules\Catalog\AvaCatalogPublisher($learningCatalog,$avaConnections,$moodleRepository,$courseProviders,$config->string('app.url'));
$iesdeLtiRobot=new \Interferencia\Modules\Catalog\IesdeLtiRobot($database,$avaConnections,$courseProviders,$rootPath);
$avaCourseProvisioning=new \Interferencia\Modules\Catalog\AvaCourseProvisioningService($database,$courseProviders,$avaCatalogPublisher,$iesdeLtiRobot);
...
$avaEnrollmentReleaser=new AvaEnrollmentReleaser($avaConnections,$moodleIntegrations,$moodleRepository,$studentEnrollments,$organizationPoles,$courseProviders,$academicOrganization,(string)$config->get('app.ava_auto_release_from'),(string)$config->get('app.encryption_key'));
...
$view->share([
    ...
    'navigation' => [
        ...
        'site' => !$isCentralContext && $auth->can('users.manage'),
