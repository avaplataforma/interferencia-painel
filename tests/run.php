...
$tests['distribui e monitora versoes do plugin Mundo Inter'] = static function () use ($rootPath): void {
    ...
    assertSame('1.0.0',$metadata['release']);
...
$tests['personaliza o AVA compartilhado pela franquia e pelo Polo Presencial'] = static function () use ($rootPath): void {
    ...
    $routes=(string)file_get_contents($rootPath.'/routes/web.php');
    $view=(string)file_get_contents($rootPath.'/views/admin/platform/painel-inter.php');
    assertTrue(str_contains($version,"\$plugin->release = '1.0.0'"));
...
$tests['publica Catalogo PRO por franquia com venda assistida'] = static function () use ($rootPath): void {
    ...
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/version.php'),'2026081801'));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/version.php'),"'1.0.1'"));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/db/services.php'),'local_mundointer_create_sso_session'));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/db/caches.php'),'sso_sessions'));
    assertTrue(!str_contains((string)file_get_contents($pluginDir.'/lib.php'),'data-support-float'));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/lib.php'),'mundointer-home-redirected'));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/lib.php'),'mundointer-welcome'));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/lib.php'),'Voltar ao site da franquia'));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/instituicao.php'),'Meus cursos'));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/instituicao.php'),'Recados'));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/sso_login.php'),'instituicao.php'));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/sso_login.php'),'brand_resolver::remember'));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/settings.php'),'ssoenabled'));
    assertTrue(str_contains((string)file_get_contents($pluginDir.'/db/upgrade.php'),'2026081801'));
...
$tests['migra o Catalogo MASTER para LTI 1.3 sem perder o legado'] = static function () use ($rootPath): void {
    ...
    assertTrue(str_contains($pluginVersion,'1.0.0'));
...
$tests['organiza novas matriculas em uma coorte por franquia e turmas no AVA'] = static function () use ($rootPath): void {
    ...
    assertTrue(str_contains($pluginVersion,"release = '1.0.0'"));
