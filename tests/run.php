...
$tests['distribui e monitora versoes do plugin Mundo Inter'] = static function () use ($rootPath): void {
    ...
    assertSame('0.9.30',$metadata['release']);
...
$tests['personaliza o AVA compartilhado pela franquia e pelo Polo Presencial'] = static function () use ($rootPath): void {
    ...
    $routes=(string)file_get_contents($rootPath.'/routes/web.php');
    $view=(string)file_get_contents($rootPath.'/views/admin/platform/painel-inter.php');
    assertTrue(str_contains($version,"\$plugin->release = '0.9.30'"));
...
$tests['migra o Catalogo MASTER para LTI 1.3 sem perder o legado'] = static function () use ($rootPath): void {
    ...
    assertTrue(str_contains($pluginVersion,'0.9.30'));
...
$tests['organiza novas matriculas em uma coorte por franquia e turmas no AVA'] = static function () use ($rootPath): void {
    ...
    assertTrue(str_contains($pluginVersion,"release = '0.9.30'"));
...
$tests['transforma disciplina master em curso individual reutilizavel'] = static function () use ($rootPath): void {
    ...
    assertTrue(str_contains($pluginExternal,"'Apostila - ' . (string)\$course->fullname"));
...
$tests['separa selecoes master por avaliacao e ignora cursos definitivos'] = static function () use ($rootPath): void {
    ...
    assertTrue(str_contains($materializer,"'Apostila em PDF'"));
    assertTrue(str_contains($materializer,"'Apostila interativa'"));
