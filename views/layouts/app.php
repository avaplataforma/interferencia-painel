...
$admOpen = preg_match('#/(users|units|roles|tags|statuses|external-forms)(?:/|$)#', $currentPath) === 1 || str_contains($currentPath, '/whatsapp/lines');
?>
<!doctype html>
<html lang="pt-BR">
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    @media(max-width:991.98px){.whatsapp-workspace{grid-template-columns:minmax(17rem,21rem) 1fr}.contact-panel{display:block;grid-column:1/-1;border-top:1px solid #e2e8ec;border-left:0}.contact-panel dl{display:grid;grid-template-columns:repeat(3,1fr);gap:1rem}.contact-panel-actions{grid-template-columns:repeat(2,minmax(0,1fr))}.conversation-lifecycle{grid-column:1/-1}}@media(max-width:575.98px){.whatsapp-heading{align-items:flex-start;flex-direction:column}.whatsapp-workspace{grid-template-columns:1fr}.chat-panel{min-height:32rem}.conversation-list{max-height:19rem}.contact-panel{grid-column:1}.contact-panel dl,.contact-panel-actions{grid-template-columns:1fr}}
...
<script src="<?= $escape($basePath) ?>/assets/js/app.js?v=6"></script>
</body>
</html>
