...
<form method="post" action="<?= $escape($basePath) ?>/users<?= $editing ? '/' . $escape($user->id) : '' ?>">
  ...
  <?php if($units!==[]):?><h2>Unidades permitidas <?= $required ?></h2><p>Selecione pelo menos uma unidade.</p><div class="checks"><?php foreach ($units as $unit): ?><label><input type="checkbox" name="units[]" value="<?= $escape($unit['id']) ?>" <?= in_array((int) $unit['id'], $selectedUnits, true) ? 'checked' : '' ?>><?= $escape($unit['name']) ?></label><?php endforeach; ?></div><?php else:?><div class="alert alert-info">Colaborador exclusivo do ADM Central, sem acesso às unidades das franquias.</div><?php endif;?>
