...
<tr>
  <td><strong><?= $escape($item['display_name']?:'Cadastro sem nome') ?></strong><small><?= $escape($item['cnpj']?:'—') ?></small></td>
  <td><strong><?= $escape($item['manager_name']?:'—') ?></strong><small><?= $escape($item['manager_email']?:'') ?></small></td>
  <td><span class="badge <?= $item['status']==='approved'?'badge-success':'badge-warning' ?>"><?= $escape($labels[$item['status']]??$item['status']) ?></span></td>
  <td><?php if(!empty($item['ticket_id'])):?><a class="application-ticket" href="<?= $escape($basePath) ?>/admin/tickets?q=F-<?= (int)$item['ticket_id'] ?>"><i class="fa-solid fa-ticket"></i> #F-<?= (int)$item['ticket_id'] ?></a><small><?= $escape($ticketLabels[$item['ticket_status']]??(string)$item['ticket_status']) ?></small><?php else:?><span class="meta">Não vinculado</span><?php endif;?></td>
  ...
  <td><a class="button button-secondary button-icon" title="Analisar solicitação" href="<?= $escape($basePath) ?>/admin/franchise-applications/<?= (int)$item['id'] ?>"><i class="fa-solid fa-eye"></i></a></td>
</tr>
<?php endforeach;?>
