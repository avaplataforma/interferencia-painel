...
<section class="card organization-section ava-commerce-manager" data-organization-panel="ava" hidden>
 <header class="organization-section-header"><span class="organization-section-icon"><i class="fa-solid fa-store"></i></span><div><h2>Oferta comercial da franquia</h2><p class="meta">Pesquise o catálogo sem carregar milhares de registros. O acesso é herdado como liberado; bloqueios e preços são exceções desta franquia.</p></div></header>

 <form class="ava-commerce-filters" method="get" action="<?= $escape($basePath) ?>/admin/organizations/<?= $organizationId ?>/edit#ava">
  <label>Catálogo<select name="ava_catalog" onchange="this.form.submit()">
   <?php foreach(($catalogAccess??[]) as$catalog):?><option value="<?= $escape((string)$catalog['code']) ?>" <?= (string)$catalog['code']===$selectedCatalogCode?'selected':'' ?>><?= $escape(str_replace('Catálogo ','',(string)$catalog['name'])) ?> · <?= (int)($catalog['inventory_count']??0) ?> curso(s) · <?= (int)($catalog['content_count']??0) ?> conteúdo(s)</option><?php endforeach;?>
  </select></label>
  <label>Tipo<select name="ava_type" onchange="this.form.submit()"><option value="course" <?= $selectedItemType==='course'?'selected':'' ?>>Cursos</option><option value="content" <?= $selectedItemType==='content'?'selected':'' ?>>Conteúdos individuais</option></select></label>
 ...
 <?php if($selectedCatalogCode==='ava-cursos'):?>
  <div class="alert alert-info"><i class="fa-solid fa-circle-info"></i> O catálogo INTER é nativo do AVA Cursos. A sincronização e os preços ficam em <strong>ADM → Cursos e preços</strong> da franquia; os demais catálogos são administrados nesta lista.</div>
 ...
 <?php else:?>
  <div class="ava-commerce-result-header"><span><strong><?= number_format((int)$inventory['total'],0,',','.') ?></strong> <?= $selectedItemType==='content'?'conteúdo(s)':'curso(s)' ?></span><span>Página <?= (int)$inventory['page'] ?> de <?= (int)$inventory['pages'] ?></span></div>
   ...
   ?>
    <details class="ava-commerce-item <?= $effectiveEnabled?'':'is-blocked' ?>">
     <summary>
      ...
      <span class="ava-commerce-title"><strong><?= $escape((string)$item['effective_name']) ?></strong><small><?= $escape((string)($item['category']?:($itemType==='content'?'Conteúdo individual':'Sem categoria'))) ?><?= (string)($item['workload']??'')!==''?' · '.$escape((string)$item['workload']):'' ?></small></span>
     ...
     <div class="ava-commerce-body">
      <div class="ava-commerce-diagnostics">
       ...
       <?php if(!$globalItemEnabled):?><span class="status-note is-danger"><i class="fa-solid fa-lock"></i> Bloqueado globalmente no catálogo central.</span><?php endif;?>
      </div>
      <form class="ava-commerce-offer-form" method="post" action="<?= $escape($basePath) ?>/admin/organizations/<?= $organizationId ?>/<?= $offerPath ?>"><?= $csrfField ?>
       ...
       <label class="ava-commerce-description">Descrição para esta franquia<textarea name="commercial_description" placeholder="Se vazio, usa a descrição aprovada no catálogo" <?= !$curated||!$effectiveEnabled?'disabled':'' ?>><?= $escape((string)($item['offer_description']??'')) ?></textarea></label>
