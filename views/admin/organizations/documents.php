...
?>
<style>
.franchise-documents{margin-top:1.25rem}.document-checklist{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.8rem}.document-check{display:flex;align-items:center;gap:.75rem;padding:1rem;border:1px solid #dce3e8;border-radius:.85rem;background:#fff}.document-check>i{display:grid;place-items:center;flex:0 0 2.35rem;width:2.35rem;height:2.35rem;border-radius:.65rem;background:#fff0f1;color:var(--inter-accent)}.document-check.is-delivered>i{background:#eaf8ef;color:#087a39}.document-check strong,.document-check small{display:block}.document-check small{margin-top:.15rem;color:#617286}.franchise-document-grid{display:grid;grid-template-columns:minmax(18rem,.8fr) minmax(0,1.2fr);gap:1rem;padding:1.25rem}.franchise-document-list{border:1px solid #e0e6ea;border-radius:.85rem;overflow:hidden}.franchise-document-list .empty-state{display:flex;min-height:12rem;flex-direction:column;align-items:center;justify-content:center;gap:.5rem;text-align:center}.franchise-document-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:1rem;align-items:center;padding:.9rem 1rem;border-bottom:1px solid #e7ebee}.franchise-document-row:last-child{border-bottom:0}.franchise-document-row strong,.franchise-document-row small{display:block}.franchise-document-actions{display:flex;gap:.4rem;align-items:center}.franchise-document-actions form{margin:0}@media(max-width:900px){.document-checklist{grid-template-columns:repeat(2,minmax(0,1fr))}.franchise-document-grid{grid-template-columns:1fr}}@media(max-width:560px){.document-checklist{grid-template-columns:1fr}}
</style>
<section class="card franchise-documents" id="documentos">
 <div class="card-header"><div><p class="eyebrow">Cadastro da franquia</p><h2><i class="fa-solid fa-folder-open"></i> Documentos</h2><p class="meta">Arquivos vinculados exclusivamente a <?= $escape((string)$organization['display_name']) ?> e armazenados de forma privada no DigitalOcean Spaces.</p></div></div>
 ...
 <div class="franchise-document-grid">
  <form class="form-grid" method="post" action="<?= $escape($documentPrefix) ?>" enctype="multipart/form-data"><?= $csrfField ?>
   <label class="form-span-2">Envio<select name="replace_id"><option value="">Novo documento</option><?php foreach($latestByGroup as$document):?><option value="<?= (int)$document['id'] ?>">Nova versão de: <?= $escape((string)$document['title']) ?> (v<?= (int)$document['version_number'] ?>)</option><?php endforeach;?></select><small>Escolha um documento existente somente quando estiver enviando uma nova versão.</small></label>
