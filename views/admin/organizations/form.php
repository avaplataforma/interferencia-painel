...
.organization-tabs{display:flex;gap:.35rem;margin:0 0 1rem;padding:.35rem;border:1px solid #dce3e8;border-radius:1rem;background:#fff;overflow-x:auto;box-shadow:0 .3rem 1rem rgb(23 33 43 / 5%)}.organization-tab{display:inline-flex;align-items:center;justify-content:center;gap:.5rem;min-width:max-content;padding:.72rem .95rem;border:0;border-radius:.72rem;background:transparent;color:#536170;font:inherit;font-weight:750;cursor:pointer}.organization-tab:hover{background:#f5f7f9;color:var(--inter-ink)}.organization-tab[aria-selected=true]{background:#fff0f1;color:var(--inter-accent);box-shadow:inset 0 0 0 1px rgb(237 28 36 / 16%)}[data-organization-panel][hidden]{display:none!important}.organization-section{min-height:25rem}.organization-savebar[hidden]{display:none!important}.organization-panel-block{grid-column:1/-1;padding:1rem;border:1px solid #e1e7eb;border-radius:.9rem;background:#f8fafb}.organization-panel-block h3,.organization-panel-block p{margin:0}.organization-panel-block p{margin-top:.25rem}.organization-site-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem}.organization-site-card{display:flex;align-items:flex-start;gap:.85rem;padding:1.1rem;border:1px solid #dfe6ea;border-radius:.9rem;background:#fff}.organization-site-card>i{display:grid;width:2.6rem;height:2.6rem;flex:0 0 2.6rem;place-items:center;border-radius:.75rem;background:#fff0f1;color:var(--inter-accent)}.organization-site-card strong,.organization-site-card small{display:block}.organization-site-card small{margin-top:.2rem;color:var(--inter-muted)}@media(max-width:800px){.organization-tabs{border-radius:.75rem}.organization-tab{padding:.65rem .8rem}.organization-section{min-height:0}.organization-site-grid{grid-template-columns:1fr}}
</style>

<div class="organization-editor">
  <div class="page-header">
    <div><p class="eyebrow">Mundo Inter · ADM Central</p><h1><?= $editing?'Editar franquia':'Cadastrar franquia' ?></h1><p>Cadastre a operação, os responsáveis e os ambientes digitais da franquia.</p></div>
  ...
  <form class="organization-editor-form" method="post" enctype="multipart/form-data" action="<?= $escape($basePath) ?>/admin/organizations<?= $editing?'/'.(int)$organization['id']:'' ?>"><?= $csrfField ?>
    <section class="card organization-section" id="dados" data-organization-panel="dados">
      ...
      <div class="organization-fields">
        ...
        <label class="field-third">Situação<select name="status"><option value="active" <?= ($organization['status']??'suspended')==='active'?'selected':'' ?>>Ativa</option><option value="suspended" <?= ($organization['status']??'suspended')!=='active'?'selected':'' ?>>Suspensa</option></select><small>A ativação libera o login exclusivo e a operação da franquia.</small></label>
        ...
        <div class="field-full organization-help"><i class="fa-solid fa-location-dot"></i><span>Endereço físico</span></div>
    ...
    <section class="card organization-section" id="painel" data-organization-panel="painel" hidden>
      <header class="organization-section-header"><span class="organization-section-icon"><i class="fa-solid fa-window-maximize"></i></span><div><h2>Painel da franquia</h2><p class="meta">Centralize o endereço de entrada e a identidade visual usada pela equipe.</p></div></header>
      <div class="organization-fields">
        <div class="organization-panel-block"><h3>Acesso ao painel</h3><p class="meta">Endereço usado pela equipe da franquia.</p></div>
        ...
        <div class="organization-panel-block"><h3>Credenciais de acesso</h3><p class="meta">Login e senha do gestor para entrar no painel da franquia.</p></div>
        <label>Login (e-mail) *<input required type="email" maxlength="190" name="panel_login" autocomplete="username" value="<?= $escape($panelUser['email']??$organization['manager_email']??'') ?>" placeholder="gestor@franquia.com.br"><small>Use o mesmo e-mail do gestor na aba Responsáveis.</small></label>
        <label>Senha<?= $editing?'':' *' ?><input <?= $editing?'':'required ' ?>type="password" minlength="8" maxlength="120" name="panel_password" autocomplete="new-password" placeholder="<?= $editing?'Deixe em branco para manter':'Mínimo de 8 caracteres' ?>"><?php if($editing&&!empty($panelUser)):?><small>O acesso atual está ativo para <?= $escape($panelUser['email']) ?>.</small><?php endif;?></label>
        <?php if(!$editing): ?><label class="field-full">Domínio público do site<input maxlength="253" name="site_host" value="<?= $escape($siteDomain['host']??'') ?>" placeholder="www.franquiatal.com.br"><small>Depois da criação, o domínio será administrado na aba Site com o passo a passo de DNS.</small></label><?php endif; ?>
        ...
        <div class="organization-panel-block"><h3>Marca da franquia</h3><p class="meta">Logo e favicon usados no painel e no site público. A própria franquia poderá trocá-los depois em ADM → Site Institucional.</p></div>
        <label>Logo<input type="file" name="logo" accept="image/png,image/jpeg,image/webp,image/svg+xml"><small>PNG, JPG ou WebP, até 3 MB.</small><?php if($editing&&!empty($organization['logo_path'])):?><span class="organization-brand-preview"><img src="<?= $escape($assetBasePath.$organization['logo_path']) ?>" alt="Logo atual"><span><strong>Logo atual</strong><label class="checkbox-row"><input type="checkbox" name="remove_logo" value="1"> Remover logo atual</label></span></span><?php endif;?></label>
        <label>Favicon<input type="file" name="favicon" accept="image/png,image/jpeg,image/webp,image/svg+xml"><small>Use uma imagem quadrada.</small><?php if($editing&&!empty($organization['favicon_path'])):?><span class="organization-brand-preview icon"><img src="<?= $escape($assetBasePath.$organization['favicon_path']) ?>" alt="Favicon atual"><span><strong>Favicon atual</strong><label class="checkbox-row"><input type="checkbox" name="remove_favicon" value="1"> Remover favicon atual</label></span></span><?php endif;?></label>
      </div>
    </section>
