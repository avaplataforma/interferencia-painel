...
<div class="document-types-page">
 <div class="page-header"><div><p class="eyebrow">ADM · Documentos</p><h1>Tipos de documentos</h1><p>Defina os documentos que os alunos da sua franquia podem anexar e quais são obrigatórios na conferência documental.</p></div></div>
 ...
 <div class="document-types-layout">
  <section class="card document-type-form">
   <p class="eyebrow"><?= $editing?'Edição':'Novo item' ?></p><h2><?= $editing?'Editar tipo':'Cadastrar tipo' ?></h2><p class="meta">O nome aparecerá na seleção de documentos do Portal do Aluno da sua franquia.</p>
   <form method="post" action="<?= $escape($basePath) ?>/admin/document-types<?= $editing?'/'.(int)$editingType['id']:'' ?>"><?= $csrfField ?>
    ...
    <div class="document-type-options">
      <label class="document-type-option"><input type="checkbox" name="is_required" value="1" <?= $editing&&!empty($editingType['is_required'])?'checked':'' ?>><span><strong>Obrigatório</strong><small>Entra na contagem de pendências do aluno.</small></span></label>
