   ...
   <tr class="content-curation-row" id="<?= $escape($editorId) ?>" hidden><td colspan="5">
    <form class="content-curation-form" method="post" enctype="multipart/form-data" action="<?= $escape($basePath) ?>/admin/platform/integrations/course-providers/contents/<?= (int)$content['id'] ?>/review"><?= $csrfField ?><input type="hidden" name="provider" value="<?= $escape($provider) ?>">
     ...
     <div class="content-curation-grid">
      ...
      <label>Liberação comercial<select name="release_status"><?php foreach($releaseLabels as$key=>$label):?><option value="<?= $key ?>" <?= $contentRelease===$key?'selected':'' ?>><?= $label ?></option><?php endforeach;?></select></label>
      <label class="span-2">Nome comercial<input name="commercial_name" maxlength="500" value="<?= $escape((string)($content['commercial_name']?:$content['name'])) ?>"></label>
      ...
      <label class="span-2">Imagem comercial por URL<input type="url" name="commercial_cover_url" value="<?= $escape((string)($content['commercial_cover_url']??'')) ?>"></label>
      <label class="span-2 catalog-image-upload">Capa otimizada no Spaces<input type="file" name="commercial_cover_file" accept="image/jpeg,image/png,image/webp"><small>JPG, PNG ou WebP. A imagem será reduzida e armazenada fora da VPS.</small><?php if(!empty($content['media_asset_id'])):?><span class="catalog-image-current"><img src="<?= $escape($basePath) ?>/catalog-media/<?= (int)$content['media_asset_id'] ?>" alt="Capa atual"><span><?= ($content['media_inheritance']??'')==='own'?'Capa própria':'Capa herdada do curso-pai' ?></span></span><?php if(($content['media_inheritance']??'')==='own'):?><span class="checkbox-row"><input type="checkbox" name="remove_cover" value="1"> Remover capa própria</span><?php endif;?><?php endif;?></label>
      <label class="span-2">Descrição comercial<textarea name="commercial_description"><?= $escape((string)($content['commercial_description']??'')) ?></textarea></label>
      <label class="span-2">Observações internas<textarea name="review_notes"><?= $escape((string)($content['review_notes']??'')) ?></textarea></label>
     </div>
