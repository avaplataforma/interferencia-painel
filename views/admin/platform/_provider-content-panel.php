<div class="catalog-subpanel" data-catalog-subpanel="<?= $escape($provider) ?>:contents" hidden>
 ...
 <div class="content-toolbar">
  <form method="get" action="<?= $escape($basePath) ?>/admin/platform/integrations/course-providers">
   <input type="hidden" name="catalog" value="conted_tech">
   ...
   <?php if($contentQuery!==''):?><a class="btn btn-secondary" href="<?= $escape($basePath) ?>/admin/platform/integrations/course-providers?catalog=conted_tech&amp;section=contents">Limpar</a><?php endif;?>
  </form>
 ...
 <?php else:?>
   ...
   <tr>
    ...
    <td><div class="content-actions">
     <details><summary><i class="fa-solid fa-pen-to-square"></i> Curadoria</summary>
      <form class="review-form" method="post" action="<?= $escape($basePath) ?>/admin/platform/integrations/course-providers/contents/<?= (int)$content['id'] ?>/review"><?= $csrfField ?>
       <div class="review-form-grid">
        ...
        <label>Categoria<input name="commercial_category" maxlength="255" value="<?= $escape((string)($content['commercial_category']??'')) ?>" placeholder="<?= $escape((string)($content['discipline_name']??'')) ?>"></label>
        <label>Carga horária<input name="commercial_workload" maxlength="100" value="<?= $escape((string)($content['commercial_workload']??'')) ?>"></label>
     ...
     <details><summary><i class="fa-solid fa-store"></i> Vender</summary>
      <form class="content-offer-form" method="post" action="<?= $escape($basePath) ?>/admin/platform/integrations/course-providers/content-offers"><?= $csrfField ?><input type="hidden" name="content_id" value="<?= (int)$content['id'] ?>">
  ...
  <?php if((int)($contentPage['pages']??1)>1):
   ...
   sort($visiblePages);
  ?><nav class="content-pagination" aria-label="Paginação de conteúdos"><?php $previousPage=0;foreach($visiblePages as$pageNumber):if($previousPage>0&&$pageNumber>$previousPage+1):?><span aria-hidden="true">…</span><?php endif;$pageUrl=$basePath.'/admin/platform/integrations/course-providers?catalog=conted_tech&section=contents&content_page='.$pageNumber.($contentQuery!==''?'&content_q='.rawurlencode($contentQuery):'');?><a class="<?= $pageNumber===$currentPage?'is-current':'' ?>" href="<?= $escape($pageUrl) ?>"><?= $pageNumber ?></a><?php $previousPage=$pageNumber;endforeach;?></nav><?php endif;?>
 <?php endif;?>
</div>
