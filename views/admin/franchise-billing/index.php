...
$deriveRule=static fn(array$item):string=>trim((string)($item['commercial_rule']??''))?:((($item['commercial_model']??'')==='fixed_plus_percentage')?((float)($item['sales_fee_percentage']??0)>0?'hybrid':'fixed_monthly'):'percentage_commission');
$deriveProcessing=static fn(array$item):string=>trim((string)($item['financial_processing']??''))?:((float)($item['sales_fee_percentage']??0)>0?'central_automatic_split':'central_monthly_settlement');
...
<div class="franchise-finance-page">
 ...
 <section class="card mb-4 franchise-finance-section"><div class="card-header"><div><p class="eyebrow">Operação recorrente</p><h2>Contratos e mensalidades</h2><p class="meta">Situação financeira e comercial por franquia.</p></div><span class="badge"><?= count($contracts) ?> contrato(s)</span></div>
  <div class="table-responsive"><table class="franchise-finance-table"><thead><tr><th>Franquia</th><th>Contrato</th><th>Regra comercial</th><th>Processamento</th><th>Mensalidade</th><th>Comissão</th><th>Requisito financeiro</th><th>Cobrança</th><th>Operação</th><th>Ações</th></tr></thead><tbody><?php foreach($contracts as$item):$rule=$deriveRule($item);$processing=$deriveProcessing($item);$walletReady=(($item['asaas_wallet_status']??'')==='validated'&&!empty($item['asaas_wallet_id'])&&(int)($item['split_enabled']??0)===1);$exclusiveReady=(($item['finance_account_mode']??'')==='exclusive'&&(int)($item['finance_integration_active']??0)===1&&($item['finance_integration_test_status']??'')==='success');$requirementReady=$processing==='central_automatic_split'?$walletReady:($processing==='franchise_asaas'?$exclusiveReady:true);?><tr>
   ...
   <td><?= $escape($rules[$rule]??'Legado') ?></td>
   <td><span class="badge"><?= $escape($processingLabels[$processing]??'Não definido') ?></span></td>
   ...
   <td><span class="badge <?= $requirementReady?'badge-success':'badge-warning' ?>"><?= $requirementReady?'Pronto':'Pendente' ?></span><small class="d-block meta"><?= $processing==='central_automatic_split'?'Wallet validada':($processing==='franchise_asaas'?'Conta exclusiva testada':'Fechamento mensal') ?></small></td>
  ...
  </tr><?php endforeach;?><?php if($contracts===[]):?><tr><td colspan="10"><div class="empty-state"><i class="fa-solid fa-magnifying-glass"></i><strong>Nenhum contrato encontrado</strong><span>Ajuste os filtros para ampliar a consulta.</span></div></td></tr><?php endif;?></tbody></table></div>
 </section>
