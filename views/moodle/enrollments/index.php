...
$eventIcons=['enrollment_created'=>'fa-user-graduate','trail_selected'=>'fa-route','charge_created'=>'fa-wallet','payment_confirmed'=>'fa-circle-check','payment_waived'=>'fa-hand-holding-heart','payment_interrupted'=>'fa-circle-xmark','ava_released'=>'fa-graduation-cap','ava_release_failed'=>'fa-triangle-exclamation','access_sent'=>'fa-paper-plane'];
?>
...
?>
<style>
.special-release{margin-bottom:1rem;overflow:hidden}.special-release>summary{display:flex;justify-content:space-between;align-items:center;gap:1rem;padding:1.15rem 1.4rem;cursor:pointer;list-style:none}.special-release>summary::-webkit-details-marker{display:none}.special-release>summary span{display:flex;align-items:center;gap:.8rem}.special-release>summary i{color:var(--inter-accent)}.special-release-body{padding:0 1.4rem 1.4rem;border-top:1px solid #e2e8ee}.special-release-option{margin-top:1rem;padding:1.2rem;border:1px solid #dbe3ea;border-radius:14px;background:#fff}.special-release-option>header{display:flex;gap:.8rem;align-items:flex-start}.special-release-option>header i{display:grid;place-items:center;width:40px;height:40px;border-radius:12px;background:#fff0f1;color:var(--inter-accent)}.special-release-option>header h3,.special-release-option>header p{margin:0}.special-release-form{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem;margin-top:1rem}.special-release-form .form-field{margin:0}.special-release-form .form-field-full{grid-column:1/-1}.special-release-actions{display:flex;justify-content:space-between;gap:1rem;align-items:center;grid-column:1/-1}.special-release-actions .alert{margin:0;flex:1}@media(max-width:760px){.special-release-form{grid-template-columns:1fr}.special-release-actions{align-items:stretch;flex-direction:column}.special-release-actions .button-primary{width:100%}}
</style>
<details class="card special-release" id="special-release">
 ...
 <div class="special-release-body">
  <section class="special-release-option"><header><i class="fa-solid fa-earth-americas"></i><div><h3>AVA Cursos</h3><p class="meta">Cursos hospedados no Moodle compartilhado ou próprio.</p></div></header>
  <form class="special-release-form" method="post" action="<?= $escape($basePath) ?>/students/enrollments/waivers" data-confirm-submit="Autorizar a liberação deste curso sem pagamento?"><?= $csrfField ?>
   <div class="form-field"><label for="waiver-unit">Unidade <span class="required">*</span></label><select id="waiver-unit" name="unit_id" required data-waiver-unit><option value="">Selecione</option><?php foreach($units as$unit):?><option value="<?= (int)$unit['id'] ?>"><?= $escape($unit['name']) ?></option><?php endforeach;?></select></div>
   <div class="form-field"><label for="waiver-student">Aluno <span class="required">*</span></label><select id="waiver-student" name="finance_customer_id" required disabled data-waiver-student><option value="">Escolha primeiro a unidade</option><?php foreach($students as$student):?><option value="<?= (int)$student['id'] ?>" data-unit-id="<?= (int)$student['unit_id'] ?>" hidden><?= $escape($student['name'].' · '.$student['unit_name'].' · '.($student['cpf_cnpj']?:'sem CPF')) ?></option><?php endforeach;?></select></div>
  ...
  <form class="special-release-form" method="post" action="<?= $escape($basePath) ?>/students/enrollments/provider-waivers" data-confirm-submit="Autorizar este acesso EXPERT sem pagamento?"><?= $csrfField ?>
   ...
   <div class="form-field form-field-full"><label for="provider-waiver-target">Curso ou conteúdo contratado <span class="required">*</span></label><select id="provider-waiver-target" name="provider_target" required><option value="">Selecione</option><?php foreach($providerTargets as$target):?><option value="<?= $escape($target['value']) ?>"><?= $escape($target['name'].' · '.$target['kind'].' · EXPERT') ?></option><?php endforeach;?></select></div>
   <div class="form-field form-field-full"><label for="provider-waiver-reason">Motivo da bolsa ou cortesia <span class="required">*</span></label><textarea id="provider-waiver-reason" name="reason" minlength="10" maxlength="500" rows="3" placeholder="Ex.: Matrícula piloto autorizada pelo ADM Central..." required></textarea><small>Obrigatório para auditoria. Entre 10 e 500 caracteres.</small></div>
   <div class="special-release-actions"><p class="alert alert-warning"><strong>Piloto:</strong> não será gerada nenhuma cobrança. O link pessoal será criado somente ao confirmar “Liberar no AVA”.</p><button class="button-primary" type="submit"><i class="fa-solid fa-flask"></i> Autorizar piloto EXPERT</button></div>
  </form>
  </section>
  <?php endif;?>
...
<thead><tr><th>Aluno</th><th>Curso</th><th>Unidade</th><th>Atendente</th><th>Financeiro</th><th>Acesso acadêmico</th><th>Ação</th></tr></thead>
<tbody>
...
<a class="button-secondary button-small" href="<?= $escape($basePath) ?>/students/<?= (int)$item['finance_customer_id'] ?>?tab=enrollments" title="Abrir cadastro unificado"><i class="fa-solid fa-eye"></i></a>
