...
final class brand_resolver
{
    ...
    public static function current(): ?array
    {
        ...
        if(isloggedin()&&!isguestuser()&&!empty($USER->id)){
            $franchisefield=(string)($catalog['franchise_field']??'mundointer_franchise');
            ...
            $franchisecode=(string)($DB->get_field_sql($sql,['userid'=>(int)$USER->id,'shortname'=>$franchisefield])?:'');
            if($franchisecode!==''&&($brand=self::byCode($brands,$franchisecode))!==null){self::remember((string)$brand['slug']);return$brand;}
    ...
    /** @return array<string,mixed>|null */
    public static function byCode(array$brands,string$code):?array{foreach($brands as$brand)if(is_array($brand)&&hash_equals((string)($brand['code']??''),$code))return$brand;return null;}
