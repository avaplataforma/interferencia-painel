...
final readonly class MessageRepository
{
    ...
    {
        $s=$this->db->prepare('SELECT m.id,m.media_id,m.file_name,m.media_attempts,m.storage_path,l.organization_id FROM whatsapp_messages m INNER JOIN whatsapp_lines l ON l.id=m.line_id WHERE m.id=:id LIMIT 1');$s->execute(['id'=>$messageId]);$row=$s->fetch();if(!is_array($row)||(string)($row['media_id']??'')===''||(string)($row['storage_path']??'')!=='')return false;
    ...
    /** @param array{mime_type:string,file_name:string,file_size:int,storage_path:string,message_type:string}|null $attachment */
    public function simulateInbound(int $lineId,string $name,string $phone,string $body,?array $attachment=null):int
    {
        ...
        $this->db->beginTransaction();try{$at=date('Y-m-d H:i:s');$s=$this->db->prepare("INSERT INTO whatsapp_conversations(line_id,wa_contact_id,contact_name,status,unread_count,is_test,last_message_at) VALUES(:line,:phone,:name,'open',1,1,:at) ON DUPLICATE KEY UPDATE contact_name=VALUES(contact_name),status='open',unread_count=unread_count+1,is_test=1,last_message_at=VALUES(last_message_at),id=LAST_INSERT_ID(id)");$s->execute(['line'=>$lineId,'phone'=>$digits,'name'=>trim($name),'at'=>$at]);$conversation=(int)$this->db->lastInsertId();$this->ensureCrmContact($conversation,$lineId,$digits,trim($name),$at);$wamid='sim_'.bin2hex(random_bytes(16));$type=$attachment['message_type']??'text';$m=$this->db->prepare("INSERT INTO whatsapp_messages(conversation_id,line_id,wamid,direction,message_type,body,media_id,mime_type,file_name,file_size,storage_path,status,message_at) VALUES(:conversation,:line,:wamid,'inbound',:type,:body,NULL,:mime,:file_name,:file_size,:storage_path,'received',:at)");$m->execute(['conversation'=>$conversation,'line'=>$lineId,'wamid'=>$wamid,'type'=>$type,'body'=>trim($body),'mime'=>$attachment['mime_type']??null,'file_name'=>$attachment['file_name']??null,'file_size'=>$attachment['file_size']??null,'storage_path'=>$attachment['storage_path']??null,'at'=>$at]);$this->db->commit();return$conversation;}catch(\Throwable$e){if($this->db->inTransaction())$this->db->rollBack();throw$e;}
    }
