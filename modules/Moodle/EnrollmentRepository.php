...
final readonly class EnrollmentRepository
{
    ...
    {
        if($units===[])return null;$marks=implode(',',array_fill(0,count($units),'?'));$s=$this->database->prepare("SELECT e.*,COALESCE(p.name,trail.name) product_name,COALESCE(e.final_value,p.value,trail_access.price_override,trail.default_price) value,COALESCE(trail_access.max_installments_override,trail.max_installments,p.max_installments,1) max_installments,c.name campaign_name FROM student_enrollments e LEFT JOIN finance_products p ON p.id=e.finance_product_id LEFT JOIN catalog_trails trail ON trail.id=e.catalog_trail_id LEFT JOIN organization_catalog_trail_access trail_access ON trail_access.organization_id=e.organization_id AND trail_access.catalog_trail_id=trail.id LEFT JOIN finance_campaigns c ON c.id=e.campaign_id WHERE e.id=? AND e.unit_id IN ($marks) AND e.finance_payment_id IS NULL AND (e.finance_product_id IS NOT NULL OR e.catalog_trail_id IS NOT NULL) LIMIT 1");$s->execute(array_merge([$id],$units));$row=$s->fetch();return is_array($row)?$row:null;
    }
