...
final readonly class MoodleRepository
{
    ...
    public function pedagogicalDashboard(array$unitIds,string$search='',string$risk='',string$lastAccessFrom='',string$lastAccessTo=''):array
    {
        $empty=['students'=>0,'enrolments'=>0,'released'=>0,'pending'=>0,'blocked'=>0,'never_accessed'=>0,'inactive_7'=>0,'inactive_15'=>0,'inactive_30'=>0,'stalled'=>0,'attention'=>0,'graded'=>0,'approved'=>0,'certificates'=>0];
        if($unitIds===[])return['summary'=>$empty,'students'=>[]];
        $marks=implode(',',array_fill(0,count($unitIds),'?'));$params=$unitIds;$where="f.unit_id IN ($marks) AND f.is_deleted=0 AND e.status<>'cancelled'";
        if($search!==''){$where.=' AND (f.name LIKE ? OR f.cpf_cnpj LIKE ? OR COALESCE(c.fullname,pco.commercial_name,pc.commercial_name,pc.name,pio.commercial_name,pci.commercial_name,pci.name) LIKE ?)';$term='%'.$search.'%';array_push($params,$term,$term,$term);}
        $sql="SELECT e.id enrollment_id,f.id customer_id,f.name,f.cpf_cnpj,u.id unit_id,u.name unit_name,COALESCE(c.fullname,pco.commercial_name,pc.commercial_name,pc.name,pio.commercial_name,pci.commercial_name,pci.name,'Formação acadêmica') course_name,e.status finance_status,e.moodle_enrolment_status,e.ava_user_id,e.academic_source,e.academic_grade_percent,e.academic_grade_status,e.academic_certificate_status,e.academic_certificate_url,e.academic_synced_at,e.academic_sync_error,me.id moodle_enrolment_id,me.is_active,me.time_start,me.time_end,COALESCE(e.academic_progress_percent,me.completion_percent) completion_percent,CASE WHEN e.academic_progress_status<>'not_available' THEN e.academic_progress_status ELSE COALESCE(me.completion_status,'not_available') END completion_status,COALESCE(e.academic_synced_at,me.progress_synced_at) progress_synced_at,me.progress_changed_at,COALESCE(e.academic_sync_error,me.progress_error) progress_error,mu.suspended,COALESCE(UNIX_TIMESTAMP(e.academic_last_access_at),CAST(JSON_UNQUOTE(JSON_EXTRACT(mu.raw_json,'$.lastaccess')) AS UNSIGNED),0) last_access FROM student_enrollments e INNER JOIN finance_customers f ON f.id=e.finance_customer_id INNER JOIN units u ON u.id=e.unit_id LEFT JOIN moodle_courses c ON c.id=e.moodle_course_id LEFT JOIN organization_provider_course_offers pco ON pco.id=e.provider_course_offer_id LEFT JOIN provider_courses pc ON pc.id=pco.provider_course_id LEFT JOIN organization_provider_content_offers pio ON pio.id=e.provider_content_offer_id LEFT JOIN provider_catalog_contents pci ON pci.id=pio.provider_content_id LEFT JOIN moodle_users mu ON mu.moodle_user_id=e.ava_user_id LEFT JOIN moodle_enrolments me ON me.moodle_user_id=e.ava_user_id AND me.moodle_course_id=c.moodle_course_id WHERE $where ORDER BY f.name,course_name LIMIT 300";
        $s=$this->database->prepare($sql);$s->execute($params);$all=$s->fetchAll();$summary=$empty;$unique=[];$students=[];$now=time();
        $fromTimestamp=preg_match('/^\d{4}-\d{2}-\d{2}$/',$lastAccessFrom)===1?strtotime($lastAccessFrom.' 00:00:00'):null;
        $toTimestamp=preg_match('/^\d{4}-\d{2}-\d{2}$/',$lastAccessTo)===1?strtotime($lastAccessTo.' 23:59:59'):null;
        foreach($all as$row){$unique[(int)$row['customer_id']]=true;$released=$row['moodle_enrolment_status']==='released';$last=(int)($row['last_access']??0);$blocked=(int)($row['suspended']??0)===1||(isset($row['is_active'])&&(int)$row['is_active']===0);$days=$last>0?(int)floor(($now-$last)/86400):null;$changed=$row['progress_changed_at']?strtotime((string)$row['progress_changed_at']):false;$stalled=$row['completion_status']==='in_progress'&&$changed!==false&&$changed<$now-(15*86400);$trackingAvailable=(string)($row['academic_source']??'')==='ava_cursos'||$last>0||!in_array((string)($row['completion_status']??'not_available'),['not_available','unavailable'],true)||$row['academic_grade_percent']!==null;$row['risk_code']=$blocked?'blocked':(!$released?'pending':(!$trackingAvailable?'unavailable':($last===0?'never_accessed':($stalled?'stalled':($days>=30?'inactive_30':($days>=15?'inactive_15':($days>=7?'inactive_7':'ok')))))));$row['risk_days']=$days;$summary['enrolments']++;$summary[$released?'released':'pending']++;if($blocked)$summary['blocked']++;if($released&&$trackingAvailable&&$last===0)$summary['never_accessed']++;if($released&&$days!==null&&$days>=7)$summary['inactive_7']++;if($released&&$days!==null&&$days>=15)$summary['inactive_15']++;if($released&&$days!==null&&$days>=30)$summary['inactive_30']++;if($stalled)$summary['stalled']++;if($row['academic_grade_percent']!==null)$summary['graded']++;if($row['academic_grade_status']==='passed')$summary['approved']++;if($row['academic_certificate_status']==='issued')$summary['certificates']++;if(in_array($row['risk_code'],['never_accessed','stalled','inactive_7','inactive_15','inactive_30'],true))$summary['attention']++;$matches=$risk===''||$risk==='all'||$row['risk_code']===$risk||($risk==='inactive_15'&&$row['risk_code']==='inactive_30')||($risk==='inactive_7'&&in_array($row['risk_code'],['inactive_15','inactive_30'],true));$matchesDate=($fromTimestamp===null&&$toTimestamp===null)||($last>0&&($fromTimestamp===null||$last>=$fromTimestamp)&&($toTimestamp===null||$last<=$toTimestamp));if($matches&&$matchesDate)$students[]=$row;}
        $summary['students']=count($unique);return['summary'=>$summary,'students'=>$students];
    }
    ...
    {
        if($unitIds===[])return[];$marks=implode(',',array_fill(0,count($unitIds),'?'));$sql="SELECT e.id student_enrollment_id,me.id moodle_enrolment_id,e.ava_connection_id,e.ava_user_id moodle_user_id,e.ava_course_id moodle_course_id,mu.username,e.organization_id,f.email,f.name student_name,mc.fullname course_name FROM student_enrollments e INNER JOIN finance_customers f ON f.id=e.finance_customer_id INNER JOIN moodle_users mu ON mu.moodle_user_id=e.ava_user_id LEFT JOIN moodle_courses mc ON mc.moodle_course_id=e.ava_course_id LEFT JOIN moodle_enrolments me ON me.moodle_user_id=e.ava_user_id AND me.moodle_course_id=e.ava_course_id WHERE e.moodle_enrolment_status='released' AND COALESCE(e.academic_provider_code,'')='' AND e.ava_user_id IS NOT NULL AND e.ava_course_id IS NOT NULL AND mu.suspended=0 AND f.is_deleted=0 AND e.unit_id IN ($marks) ORDER BY COALESCE(e.academic_synced_at,'2000-01-01') LIMIT ".max(1,min(300,$limit));$s=$this->database->prepare($sql);$s->execute($unitIds);return$s->fetchAll();
    }
    ...
    public function saveAcademicSnapshot(int$studentEnrollmentId,?int$moodleEnrolmentId,array$snapshot,?string$error=null):void
    {
        $progress=$this->nullablePercent($snapshot['progresspercent']??null);$grade=$this->nullablePercent($snapshot['gradepercent']??null);$last=(int)($snapshot['lastaccess']??0);
        $s=$this->database->prepare("UPDATE student_enrollments SET academic_source=:source,academic_progress_percent=:progress,academic_progress_status=:progress_status,academic_grade_percent=:grade,academic_grade_status=:grade_status,academic_last_access_at=:last_access,academic_certificate_status=:certificate_status,academic_certificate_url=:certificate_url,academic_synced_at=NOW(),academic_sync_error=:error WHERE id=:id");
        $s->execute(['source'=>(string)($snapshot['provider']??'ava_cursos'),'progress'=>$progress,'progress_status'=>(string)($snapshot['progressstatus']??'not_available'),'grade'=>$grade,'grade_status'=>(string)($snapshot['gradestatus']??'not_available'),'last_access'=>$last>0?date('Y-m-d H:i:s',$last):null,'certificate_status'=>(string)($snapshot['certificatestatus']??'not_available'),'certificate_url'=>$this->nullable($snapshot['certificateurl']??null),'error'=>$error,'id'=>$studentEnrollmentId]);
        if($moodleEnrolmentId!==null&&$moodleEnrolmentId>0)$this->saveProgress($moodleEnrolmentId,$progress,(string)($snapshot['progressstatus']??'not_available'),$error);
    }
    ...
    private function nullablePercent(mixed$value):?float{$value=is_numeric($value)?(float)$value:-1;return$value<0?null:max(0,min(100,$value));}
