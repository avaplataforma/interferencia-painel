...
final readonly class MoodleRepository
{
    ...
    {
        if($unitIds===[])return[];$marks=implode(',',array_fill(0,count($unitIds),'?'));$sql="SELECT e.id student_enrollment_id,me.id moodle_enrolment_id,e.ava_connection_id,e.ava_user_id moodle_user_id,e.ava_course_id moodle_course_id,mu.username FROM student_enrollments e INNER JOIN finance_customers f ON f.id=e.finance_customer_id INNER JOIN moodle_users mu ON mu.moodle_user_id=e.ava_user_id LEFT JOIN moodle_enrolments me ON me.moodle_user_id=e.ava_user_id AND me.moodle_course_id=e.ava_course_id WHERE e.moodle_enrolment_status='released' AND COALESCE(e.academic_provider_code,'')='' AND e.ava_user_id IS NOT NULL AND e.ava_course_id IS NOT NULL AND mu.suspended=0 AND f.is_deleted=0 AND e.unit_id IN ($marks) ORDER BY COALESCE(e.academic_synced_at,'2000-01-01') LIMIT ".max(1,min(300,$limit));$s=$this->database->prepare($sql);$s->execute($unitIds);return$s->fetchAll();
    }
