...
final readonly class TicketRepository
{
    ...
    public function all(int$userId,bool$manage,array$unitIds,string$scope='',string$status='',string$priority='',string$search=''):array
    {
        ...
        $sql="SELECT t.*,un.name unit_name,requester.name requester_name,department.name department_name,contact.name contact_name,(tr.last_read_at IS NULL OR t.updated_at>tr.last_read_at) unread FROM tickets t INNER JOIN units un ON un.id=t.unit_id INNER JOIN users requester ON requester.id=t.requester_user_id INNER JOIN ticket_departments department ON department.id=t.department_id LEFT JOIN crm_contacts contact ON contact.id=t.crm_contact_id LEFT JOIN ticket_reads tr ON tr.ticket_id=t.id AND tr.user_id=? WHERE t.unit_id IN ($marks)";$params=[$userId,...$unitIds];
        ...
        if($search!==''){$sql.=' AND (t.subject LIKE ? OR t.description LIKE ? OR requester.name LIKE ? OR department.name LIKE ? OR contact.name LIKE ?)';$term='%'.$search.'%';array_push($params,$term,$term,$term,$term,$term);}
    ...
    public function find(int$id,int$userId,bool$manage,array$unitIds):?array
    {
        ...
        $sql="SELECT t.*,un.name unit_name,requester.name requester_name,department.name department_name,contact.name contact_name,contact.phone contact_phone FROM tickets t INNER JOIN units un ON un.id=t.unit_id INNER JOIN users requester ON requester.id=t.requester_user_id INNER JOIN ticket_departments department ON department.id=t.department_id LEFT JOIN crm_contacts contact ON contact.id=t.crm_contact_id WHERE t.id=? AND t.unit_id IN ($marks)";$params=[$id,...$unitIds];
    ...
    public function create(int$unitId,int$requesterId,int$departmentId,int$contactId,string$subject,string$description,string$priority,?string$dueAt):int
    {
        $this->db->beginTransaction();try{$s=$this->db->prepare("INSERT INTO tickets(unit_id,crm_contact_id,department_id,requester_user_id,assigned_user_id,subject,description,priority,status,due_at) VALUES(:unit,:contact,:department,:requester,NULL,:subject,:description,:priority,'open',:due)");$s->execute(['unit'=>$unitId,'contact'=>$contactId,'department'=>$departmentId,'requester'=>$requesterId,'subject'=>trim($subject),'description'=>trim($description),'priority'=>$priority,'due'=>$dueAt]);$id=(int)$this->db->lastInsertId();$this->event($id,$requesterId,'created',null,'Chamado aberto');$this->markRead($id,$requesterId);$this->db->commit();return$id;}catch(\Throwable$e){if($this->db->inTransaction())$this->db->rollBack();throw$e;}
    }
