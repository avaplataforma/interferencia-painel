...
final readonly class PlatformBrandingStorage
{
    ...
    public function store(UploadedFile$file,string$kind):string
    {
        ...
        $contents=file_get_contents($file->temporaryPath);if(!is_string($contents))throw new RuntimeException('Não foi possível ler a imagem.');$this->spaces?->storeCentral('Personalizacao',$contents,$kind.'.'.$extensions[$mime],$mime);
