...
final readonly class FranchiseContractRepository
{
    ...
    {
        return $this->db->query("SELECT c.*,a.display_name franchise_name,a.cnpj,o.asaas_wallet_id,o.asaas_wallet_status,o.split_enabled,ofi.account_mode finance_account_mode,ofi.is_active finance_integration_active,ofi.last_test_status finance_integration_test_status FROM franchise_contracts c INNER JOIN franchise_applications a ON a.id=c.franchise_application_id LEFT JOIN organizations o ON o.id=c.organization_id LEFT JOIN organization_finance_integrations ofi ON ofi.organization_id=o.id AND ofi.provider='asaas' ORDER BY c.updated_at DESC")->fetchAll();
    }
    ...
    {
        $s=$this->db->prepare("SELECT c.*,a.display_name franchise_name,a.legal_name,a.cnpj,a.manager_name,a.manager_email,a.manager_phone,a.postal_code,a.address,a.address_number,a.address_complement,a.neighborhood,o.asaas_wallet_id,o.asaas_wallet_status,o.split_enabled,ofi.account_mode finance_account_mode,ofi.is_active finance_integration_active,ofi.last_test_status finance_integration_test_status,pu.name cancelled_by_name FROM franchise_contracts c INNER JOIN franchise_applications a ON a.id=c.franchise_application_id LEFT JOIN organizations o ON o.id=c.organization_id LEFT JOIN organization_finance_integrations ofi ON ofi.organization_id=o.id AND ofi.provider='asaas' LEFT JOIN platform_users pu ON pu.id=c.cancelled_by WHERE c.id=:id");$s->execute(['id'=>$id]);$row=$s->fetch();return is_array($row)?$row:null;
    }
    ...
    public function create(int $applicationId,int $templateId,array $data,?int $userId): int
    {
        ...
        $rule=(string)($data['commercial_rule']??'');
        $processing=(string)($data['financial_processing']??'');
        if(!in_array($rule,['percentage_commission','fixed_monthly','hybrid','per_enrollment'],true))throw new RuntimeException('Selecione a regra comercial do contrato.');
        if(!in_array($processing,['central_monthly_settlement','central_automatic_split','franchise_asaas'],true))throw new RuntimeException('Selecione como o pagamento será processado.');
        $monthly=self::money($data['monthly_fixed_amount']??null)??0.0;
        $percentage=self::percentage($data['sales_fee_percentage']??null)??0.0;
        $franchisePercentage=self::percentage($data['franchise_fee_percentage']??null)??0.0;
        $perEnrollment=self::money($data['fixed_fee_per_enrollment']??null)??0.0;
        $percentageRule=in_array($rule,['percentage_commission','hybrid'],true);
        if(in_array($rule,['fixed_monthly','hybrid'],true)&&$monthly<=0)throw new RuntimeException('Informe o valor da mensalidade fixa.');
        if($rule==='per_enrollment'&&$perEnrollment<=0)throw new RuntimeException('Informe o valor fixo por matrícula.');
        if($percentageRule&&($percentage<=0||$franchisePercentage<=0||abs(($percentage+$franchisePercentage)-100)>0.01))throw new RuntimeException('Os percentuais do Mundo Inter e da franquia devem ser maiores que zero e somar 100%.');
        if(!$percentageRule){$percentage=0.0;$franchisePercentage=0.0;}
        if($processing==='central_automatic_split'&&!$percentageRule)throw new RuntimeException('O split automático exige uma regra comercial com percentuais por venda.');
        $closingDay=self::day($data['closing_day']??null);
        $settlementDay=self::day($data['settlement_day']??null);
        if($processing==='central_monthly_settlement'&&($closingDay===null||$settlementDay===null))throw new RuntimeException('Informe os dias de fechamento e de repasse mensal.');
        $model=in_array($rule,['fixed_monthly','hybrid'],true)?'fixed_plus_percentage':'split_only';
        ...
        $billing=$monthly>0;$description=self::nullable($data['billing_description']??'Licenciamento mensal Mundo Inter');
        $s=$this->db->prepare("INSERT INTO franchise_contracts(parent_contract_id,contract_number,contract_type,valid_from,valid_until,commercial_model,commercial_rule,financial_processing,monthly_fixed_amount,sales_fee_percentage,franchise_fee_percentage,fixed_fee_per_enrollment,closing_day,settlement_day,franchise_application_id,organization_id,template_id,title,content,public_token,status,billing_required,billing_amount,billing_description,created_by) VALUES(:parent,:number,:type,:valid_from,:valid_until,:model,:rule,:processing,:monthly,:percentage,:franchise_percentage,:per_enrollment,:closing_day,:settlement_day,:application,:organization,:template,:title,:content,:token,'draft',:billing,:amount,:description,:created_by)");
        $s->execute(['parent'=>$parent['id']??null,'number'=>$number,'type'=>$type,'valid_from'=>$validFrom,'valid_until'=>$validUntil,'model'=>$model,'rule'=>$rule,'processing'=>$processing,'monthly'=>$monthly>0?$monthly:null,'percentage'=>$percentage,'franchise_percentage'=>$franchisePercentage,'per_enrollment'=>$perEnrollment>0?$perEnrollment:null,'closing_day'=>$closingDay,'settlement_day'=>$settlementDay,'application'=>$applicationId,'organization'=>$a['organization_id'],'template'=>$templateId,'title'=>$title,'content'=>$content,'token'=>bin2hex(random_bytes(32)),'billing'=>$billing?1:0,'amount'=>$billing?$monthly:null,'description'=>$description,'created_by'=>$userId]);
    ...
    public function billingDashboard(array$filters=[]): array
    {
        ...
        $contractsStatement=$this->db->prepare("SELECT c.*,a.display_name franchise_name,o.asaas_wallet_status,o.asaas_wallet_id,o.split_enabled,ofi.account_mode finance_account_mode,ofi.is_active finance_integration_active,ofi.last_test_status finance_integration_test_status FROM franchise_contracts c INNER JOIN franchise_applications a ON a.id=c.franchise_application_id LEFT JOIN organizations o ON o.id=c.organization_id LEFT JOIN organization_finance_integrations ofi ON ofi.organization_id=o.id AND ofi.provider='asaas'".$clause." ORDER BY FIELD(c.commercial_flow_status,'blocked','pending','active','inactive'),c.updated_at DESC LIMIT 100");$contractsStatement->execute($params);$contracts=$contractsStatement->fetchAll();
    ...
    public function prepareSplit(int $organizationId,float $grossValue,string $externalReference): ?array
    {
        ...
        if(!is_array($contract)||self::processing($contract)!=='central_automatic_split')return null;$central=round((float)$contract['sales_fee_percentage'],4);if($central<=0)return null;
        ...
        $franchise=round((float)($contract['franchise_fee_percentage']??(100-$central)),4);if($franchise<=0||abs(($central+$franchise)-100)>0.01)throw new RuntimeException('Os percentuais contratados não formam uma divisão válida.');
    ...
    private static function day(mixed$value):?int{$value=trim((string)$value);if($value==='')return null;if(!ctype_digit($value))throw new RuntimeException('Informe dias de fechamento e repasse entre 1 e 31.');$day=(int)$value;if($day<1||$day>31)throw new RuntimeException('Informe dias de fechamento e repasse entre 1 e 31.');return$day;}
    ...
    private static function processing(array$contract):string{$value=trim((string)($contract['financial_processing']??''));if($value!=='')return$value;return(float)($contract['sales_fee_percentage']??0)>0?'central_automatic_split':'central_monthly_settlement';}
    private static function processingLabel(string$value):string{return['central_monthly_settlement'=>'central com fechamento e repasse mensal','central_automatic_split'=>'central com split automático','franchise_asaas'=>'pela conta Asaas exclusiva da franquia','external_gateway'=>'por gateway externo'][$value]??$value;}
}
