...
final readonly class FranchiseSandboxBillingRepository
{
    ...
    {
        return $this->db->query("SELECT c.id,c.organization_id,c.commercial_model,c.monthly_fixed_amount,c.sales_fee_percentage,a.display_name franchise_name,a.legal_name,a.cnpj,a.manager_name,a.manager_email,a.manager_phone FROM franchise_contracts c INNER JOIN franchise_applications a ON a.id=c.franchise_application_id WHERE c.status='signed' ORDER BY a.display_name,c.contract_number DESC")->fetchAll();
    }

    public function contract(int $id): array
    {
        $s=$this->db->prepare("SELECT c.id,c.organization_id,c.status,c.commercial_model,c.monthly_fixed_amount,c.sales_fee_percentage,a.display_name franchise_name,a.legal_name,a.cnpj,a.manager_name,a.manager_email,a.manager_phone FROM franchise_contracts c INNER JOIN franchise_applications a ON a.id=c.franchise_application_id WHERE c.id=:id");
        $s->execute(['id'=>$id]);$row=$s->fetch();
        if(!is_array($row)||$row['status']!=='signed')throw new RuntimeException('Selecione uma franquia com contrato assinado.');
        return$row;
    }

    public function begin(array $contract,?int $userId): array
    {
        $central=max(0,min(100,round((float)$contract['sales_fee_percentage'],4)));$franchise=round(100-$central,4);$gross=5.00;
