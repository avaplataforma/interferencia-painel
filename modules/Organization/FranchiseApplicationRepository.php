...
final readonly class FranchiseApplicationRepository
{
    ...
    {
        $s=$this->db->prepare('SELECT a.*,o.display_name organization_name,(SELECT p.id FROM platform_tickets p WHERE p.franchise_application_id=a.id ORDER BY p.id LIMIT 1) ticket_id,(SELECT p.status FROM platform_tickets p WHERE p.franchise_application_id=a.id ORDER BY p.id LIMIT 1) ticket_status FROM franchise_applications a LEFT JOIN organizations o ON o.id=a.organization_id WHERE a.id=:id');$s->execute(['id'=>$id]);$row=$s->fetch();return is_array($row)?$row:null;
    }
    ...
    public function submit(string $token,array $data): int
    {
        ...
        $payload=$this->validatedPayload($data);$payload['id']=(int)$record['id'];
        ...
        try{
            ...
            $this->createTicket((int)$record['id'],$payload);
    ...
    public function submitNew(array$data):int
    {
        $payload=$this->validatedPayload($data);$payload['token']=bin2hex(random_bytes(32));
        $this->db->beginTransaction();
        try{
            $s=$this->db->prepare("INSERT INTO franchise_applications(public_token,display_name,legal_name,cnpj,state_registration,municipal_registration,postal_code,address,address_number,address_complement,neighborhood,city,state,manager_name,manager_document,manager_email,manager_phone,general_manager_name,general_manager_email,general_manager_phone,site_host,negotiation_notes,billing_required,status,submitted_at) VALUES(:token,:display,:legal,:cnpj,:state_registration,:municipal_registration,:postal_code,:address,:address_number,:address_complement,:neighborhood,:city,:state,:manager,:manager_document,:manager_email,:manager_phone,:general_manager_name,:general_manager_email,:general_manager_phone,:site_host,:notes,:billing_required,'submitted',NOW())");
            $s->execute($payload);$id=(int)$this->db->lastInsertId();$this->createTicket($id,$payload);$this->db->commit();return$id;
        }catch(Throwable$e){if($this->db->inTransaction())$this->db->rollBack();if($e instanceof RuntimeException)throw$e;throw new RuntimeException('Não foi possível enviar. Verifique se o CNPJ já foi cadastrado.',0,$e);}
    }
    ...
    private function validatedPayload(array$data):array
    {
        ...
        return['display'=>$display,'legal'=>$legal,'cnpj'=>$cnpj,'state_registration'=>self::nullable($data['state_registration']??''),'municipal_registration'=>self::nullable($data['municipal_registration']??''),'postal_code'=>self::nullable($data['postal_code']??''),'address'=>self::nullable($data['address']??''),'address_number'=>self::nullable($data['address_number']??''),'address_complement'=>self::nullable($data['address_complement']??''),'neighborhood'=>self::nullable($data['neighborhood']??''),'city'=>self::nullable($data['city']??''),'state'=>$state!==''?$state:null,'manager'=>$manager,'manager_document'=>$managerDocument!==''?$managerDocument:null,'manager_email'=>$email,'manager_phone'=>$phone,'general_manager_name'=>self::nullable($data['general_manager_name']??''),'general_manager_email'=>$generalEmail!==''?$generalEmail:null,'general_manager_phone'=>self::nullable($data['general_manager_phone']??''),'site_host'=>$host!==''?OrganizationRepository::normalizeHost($host):null,'notes'=>self::nullable($data['negotiation_notes']??''),'billing_required'=>0];
    }

    private function createTicket(int$applicationId,array$payload):void
    {
        $description="Nova solicitação de franquia recebida pelo cadastro público.\n\nFranquia: {$payload['display']}\nRazão social: {$payload['legal']}\nCNPJ: {$payload['cnpj']}\nGestor: {$payload['manager']}\nE-mail: {$payload['manager_email']}\nTelefone: {$payload['manager_phone']}\n\nPróximas etapas: conferir os dados, definir o modelo de negócio, gerar o contrato e concluir a ativação.";
        $t=$this->db->prepare("INSERT INTO platform_tickets(franchise_application_id,subject,requester_name,requester_email,description,priority,status) VALUES(:application,:subject,:name,:email,:description,'high','open')");$t->execute(['application'=>$applicationId,'subject'=>'Concluir cadastro de franquia — '.$payload['display'],'name'=>$payload['manager'],'email'=>$payload['manager_email'],'description'=>$description]);
    }
}
