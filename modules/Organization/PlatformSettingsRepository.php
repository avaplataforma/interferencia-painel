...
final readonly class PlatformSettingsRepository
{
    ...
    public function updateBrandingPaths(?string$logo,?string$favicon):void{$s=$this->database->prepare('UPDATE platform_settings SET logo_path=:logo,favicon_path=:favicon WHERE id=1');$s->execute(['logo'=>$logo??'/assets/media/mundo-inter-logo.png?v=20260806','favicon'=>$favicon??'/assets/media/mundo-inter-favicon.png?v=20260806']);}
