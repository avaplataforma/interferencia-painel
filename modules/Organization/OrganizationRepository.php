...
final readonly class OrganizationRepository
{
    ...
    public function save(?int$id,array$data):int
    {
        ...
        $identity=['code'=>$code,'slug'=>$panelSlug,'legal'=>$legal,'display'=>$display,'status'=>$status,'primary'=>$primary,'secondary'=>$secondary,'login_title'=>$loginTitle!==''?$loginTitle:null,'welcome'=>$welcome!==''?$welcome:null,'email'=>$email!==''?$email:null,'phone'=>$phone!==''?$phone:null];
        $this->database->beginTransaction();
        try{
            if($id===null){$s=$this->database->prepare('INSERT INTO organizations(public_id,code,panel_slug,legal_name,display_name,status,primary_color,secondary_color,login_title,login_welcome_text,support_email,support_phone) VALUES(UUID(),:code,:slug,:legal,:display,:status,:primary,:secondary,:login_title,:welcome,:email,:phone)');$s->execute($identity);$id=(int)$this->database->lastInsertId();}
            else{$s=$this->database->prepare('UPDATE organizations SET code=:code,panel_slug=:slug,legal_name=:legal,display_name=:display,status=:status,primary_color=:primary,secondary_color=:secondary,login_title=:login_title,login_welcome_text=:welcome,support_email=:email,support_phone=:phone WHERE id=:id');$s->execute($identity+['id'=>$id]);if($this->findRecord($id)===null)throw new RuntimeException('Organização não encontrada.');}
