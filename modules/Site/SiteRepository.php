...
final readonly class SiteRepository
{
    ...
    public function externalProducts(int $organizationId):array
    {
        ...
        foreach($products as&$product){$product['is_external']=1;$product['product_kind']='provider_course';$product['modality']=($product['execution_environment']??'')==='shared_ava'?'AVA Cursos':'AVA do fornecedor';$product['workload_hours']=(int)(preg_replace('/\D+/', '', (string)($product['workload_text']??''))?:0);$product['billing_types']='[]';$product['minutes_to_expire']=0;$product['seo_title']=$product['name'];$product['seo_description']=$product['description'];}
    ...
    public function publicSite(int $organizationId,bool $preview=false):?array
    {
        ...
        $externalProducts=array_merge($this->externalProducts($organizationId),$this->externalContentProducts($organizationId));
