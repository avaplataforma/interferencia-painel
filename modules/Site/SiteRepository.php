...
final readonly class SiteRepository
{
    ...
    {
        $statement=$this->database->prepare("SELECT offer.id,offer.organization_id,offer.price value,offer.max_installments,offer.sale_mode,COALESCE(NULLIF(offer.commercial_name,''),NULLIF(course.commercial_name,''),course.name) name,COALESCE(NULLIF(offer.commercial_description,''),NULLIF(course.commercial_description,''),course.description) description,COALESCE(NULLIF(course.commercial_category,''),course.category) category,COALESCE(NULLIF(course.commercial_workload,''),course.workload) workload_text,course.lesson_count,COALESCE(NULLIF(course.commercial_cover_url,''),course.cover_url) cover_url,asset.id media_asset_id,catalog.name catalog_name,catalog.code catalog_code,provider.name provider_name,provider.delivery_mode,provider.launch_url_template,course.remote_id,course.external_key FROM organization_provider_course_offers offer INNER JOIN provider_courses course ON course.id=offer.provider_course_id INNER JOIN course_catalogs catalog ON catalog.id=course.catalog_id LEFT JOIN organization_course_catalog_access access ON access.course_catalog_id=catalog.id AND access.organization_id=offer.organization_id LEFT JOIN organization_catalog_item_access item_access ON item_access.organization_id=offer.organization_id AND item_access.item_type='course' AND item_access.item_id=course.id INNER JOIN course_provider_integrations provider ON provider.id=course.provider_id LEFT JOIN catalog_media_assets asset ON asset.entity_type='course' AND asset.entity_id=course.id AND asset.purpose='cover' AND asset.generation_status='ready' WHERE offer.organization_id=:organization AND offer.is_active=1 AND offer.is_visible=1 AND course.review_status='approved' AND course.release_status IN ('released','published') AND course.is_available=1 AND course.is_globally_enabled=1 AND catalog.is_active=1 AND catalog.is_globally_enabled=1 AND COALESCE(access.is_enabled,1)=1 AND COALESCE(item_access.is_enabled,1)=1 AND provider.is_active=1 ORDER BY catalog.name,name");
