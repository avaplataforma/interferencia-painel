...
final readonly class ExternalContactIntake{
 ...
 public function receive(string $providedKey,array $data,string $source='unknown'):array{
  ...
  $phone=$this->nullable($data['telefone']??null);$email=$this->nullable($data['email']??null);if($phone===null&&$email===null)throw new RuntimeException('Informe telefone ou e-mail.',422);if($phone!==null&&!in_array(strlen((string)preg_replace('/\D/','',$phone)),[10,11],true))throw new RuntimeException('Telefone inválido.',422);if($email!==null&&filter_var($email,FILTER_VALIDATE_EMAIL)===false)throw new RuntimeException('E-mail inválido.',422);$course=$this->nullable($data['curso']??null);if($course===null)throw new RuntimeException('Informe o curso de interesse.',422);
  ...
  $id=$this->contacts->createExternal(['unit_id'=>(int)$unit['id'],'name'=>$name,'phone'=>$phone,'email'=>$email,'course'=>$course,'interest_score'=>$score,'origin_city'=>(string)$unit['name'],'external_submission_id'=>$submission,'consent_at'=>$consent?->format('Y-m-d H:i:s'),'privacy_notice_version'=>$this->nullable($data['versao_privacidade']??null),'registered_at'=>$registered->format('Y-m-d H:i:s'),'notes'=>$this->nullable($data['observacoes']??null)]);return['id'=>$id,'duplicate'=>false];
 }
