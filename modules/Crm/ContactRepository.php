...
final readonly class ContactRepository
{
    ...
    public function createExternal(array $data):int{$sql="INSERT INTO crm_contacts (organization_id,unit_id,status_id,name,phone,email,document,course,interest_score,origin_city,registration_source,external_submission_id,consent_at,privacy_notice_version,landing_page,utm_source,utm_medium,utm_campaign,utm_content,utm_term,registered_at,notes,is_active) SELECT :organization,:unit_id,id,:name,:phone,:email,:document,:course,:interest_score,:origin_city,'external_form',:external_submission_id,:consent_at,:privacy_notice_version,:landing_page,:utm_source,:utm_medium,:utm_campaign,:utm_content,:utm_term,:registered_at,:notes,1 FROM crm_statuses WHERE code='new' AND organization_id=:organization_status";$s=$this->database->prepare($sql);$data+=['document'=>null,'landing_page'=>null,'utm_source'=>null,'utm_medium'=>null,'utm_campaign'=>null,'utm_content'=>null,'utm_term'=>null,'organization'=>$this->organizationId,'organization_status'=>$this->organizationId];$s->execute($data);$id=(int)$this->database->lastInsertId();$this->recordEvent($id,null,'created','Contato recebido por site externo.');return$id;}
