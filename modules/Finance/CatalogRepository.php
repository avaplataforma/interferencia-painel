...
final readonly class CatalogRepository
{
    ...
    public function save(?int$id,?int$unitId,string$name,?string$description,float$value,array$billingTypes,int$minutes,bool$active):int
    {
        $billing=implode(',',array_values(array_intersect(['PIX','CREDIT_CARD'],$billingTypes)));if($id===null){$s=$this->database->prepare('INSERT INTO finance_products(unit_id,name,description,value,billing_types,minutes_to_expire,is_active) VALUES(:unit,:name,:description,:value,:billing,:minutes,:active)');$s->execute(['unit'=>$unitId,'name'=>$name,'description'=>$description,'value'=>$value,'billing'=>$billing,'minutes'=>$minutes,'active'=>(int)$active]);return(int)$this->database->lastInsertId();}$s=$this->database->prepare('UPDATE finance_products SET unit_id=:unit,name=:name,description=:description,value=:value,billing_types=:billing,minutes_to_expire=:minutes,is_active=:active WHERE id=:id');$s->execute(['unit'=>$unitId,'name'=>$name,'description'=>$description,'value'=>$value,'billing'=>$billing,'minutes'=>$minutes,'active'=>(int)$active,'id'=>$id]);return$id;
    }
