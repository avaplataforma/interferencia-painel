...
final readonly class FinanceRepository
{
    ...
    {
        $customer=(string)($item['customer']??'');$q=$this->database->prepare('SELECT id,unit_id FROM finance_customers WHERE asaas_customer_id=:id AND unit_id IS NOT NULL');$q->execute(['id'=>$customer]);$linked=$q->fetch();if(!is_array($linked))throw new \RuntimeException('Cliente da assinatura não está conciliado com uma unidade.');
