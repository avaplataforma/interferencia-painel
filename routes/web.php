...
return static function (
    ...
    $router->postWithoutCsrf('/api/asaas/webhook',static function(Request$request)use($asaasWebhook,$finance,$financeCatalog):Response{
    ...
    $router->post('/finance/customers/{id:\d+}/edit',static function(Request$request,array$params)use($finance,$financeScope,$auth,$asaas,$session,$basePath):Response{$id=(int)$params['id'];try{$customer=$finance->customer($id,$financeScope(),$auth->can('finance.legacy_view'));if($customer===null)throw new RuntimeException('Cliente financeiro não encontrado.');$name=trim((string)$request->input('name',''));$email=strtolower(trim((string)$request->input('email','')));$document=preg_replace('/\D/','',(string)$request->input('cpf_cnpj',''))??'';$phone=preg_replace('/\D/','',(string)$request->input('phone',''))??'';$mobile=preg_replace('/\D/','',(string)$request->input('mobile_phone',''))??'';if(mb_strlen($name)<2||mb_strlen($name)>160)throw new RuntimeException('Informe um nome válido.');if(filter_var($email,FILTER_VALIDATE_EMAIL)===false||mb_strlen($email)>190)throw new RuntimeException('Informe um e-mail válido.');if($document!==''&&!in_array(strlen($document),[11,14],true))throw new RuntimeException('Informe um CPF ou CNPJ válido.');if($phone!==''&&(strlen($phone)<10||strlen($phone)>11))throw new RuntimeException('Informe um telefone válido.');if($mobile!==''&&(strlen($mobile)<10||strlen($mobile)>11))throw new RuntimeException('Informe um celular válido.');$changes=[];foreach(['name'=>$name,'email'=>$email,'cpfCnpj'=>$document,'phone'=>$phone,'mobilePhone'=>$mobile]as$field=>$value){$localField=match($field){'cpfCnpj'=>'cpf_cnpj','mobilePhone'=>'mobile_phone',default=>$field};if($value!==''&&$value!==(string)($customer[$localField]??''))$changes[$field]=$value;}if($changes===[])throw new RuntimeException('Nenhuma alteração foi informada.');$updated=$asaas->updateCustomer((string)$customer['asaas_customer_id'],$changes);$finance->upsertCustomer($updated);$session->flash('finance_customers.message','Cadastro do cliente atualizado no Painel e no Asaas.');return Response::redirect($basePath.'/finance/customers/'.$id);}catch(Throwable$e){$session->flash('finance_customers.edit_error',$e->getMessage());return Response::redirect($basePath.'/finance/customers/'.$id.'/edit');}},$manageFinanceCustomers);
    ...
    $saveProduct=static function(Request$request,?int$id=null)use($financeCatalog,$units,$session,$basePath):Response{try{$name=trim((string)$request->input('name',''));if(mb_strlen($name)<2||mb_strlen($name)>160)throw new RuntimeException('Informe o nome do curso com até 160 caracteres.');$description=trim((string)$request->input('description',''));if(mb_strlen($description)>500)throw new RuntimeException('A descrição deve possuir até 500 caracteres.');$value=round((float)str_replace(',','.',trim((string)$request->input('value',''))),2);if($value<5)throw new RuntimeException('O valor mínimo é R$ 5,00.');$unitRaw=(string)$request->input('unit_id','');$unitId=$unitRaw===''?null:(int)$unitRaw;if($unitId!==null&&$units->find($unitId)===null)throw new RuntimeException('Selecione uma unidade válida.');$rawBilling=$request->input('billing_types',[]);$billing=is_array($rawBilling)?array_map('strval',$rawBilling):[];if(array_intersect($billing,['PIX','CREDIT_CARD'])===[])throw new RuntimeException('Selecione Pix e/ou cartão.');$minutes=(int)$request->input('minutes_to_expire','1440');if($minutes<10||$minutes>1440)throw new RuntimeException('A validade deve ficar entre 10 e 1.440 minutos.');$financeCatalog->save($id,$unitId,$name,$description?:null,$value,$billing,$minutes,$request->input('is_active')==='1');$session->flash('finance_products.message',$id===null?'Curso e preço cadastrados.':'Curso e preço atualizados.');return Response::redirect($basePath.'/admin/finance/products');}catch(Throwable$e){$session->flash('finance_products.error',$e->getMessage());return Response::redirect($basePath.($id===null?'/admin/finance/products/create':"/admin/finance/products/{$id}/edit"));}};
