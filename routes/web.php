...
return static function (
    ...
    $router->get('/whatsapp/attachments/{id:\d+}',static function(Request$request,array$params)use($whatsappLines,$whatsappMessages,$whatsappMedia,$auth):Response{$lineIds=array_map(static fn(array$l):int=>(int)$l['id'],$whatsappLines->authorizedForUser($auth->user()->id));$attachment=$whatsappMessages->attachment((int)$params['id'],$lineIds);if($attachment===null)return Response::text("Anexo não encontrado.\n",404);try{$body=$whatsappMedia->read((string)$attachment['storage_path']);$name=preg_replace('/[^\pL\pN._ -]+/u','_',basename((string)$attachment['file_name']))?:'anexo';return(new Response($body,200))->withHeaders(['Content-Type'=>(string)$attachment['mime_type'],'Content-Length'=>(string)strlen($body),'Content-Disposition'=>'attachment; filename="'.str_replace('"','',$name).'"','X-Content-Type-Options'=>'nosniff','Cache-Control'=>'private, no-store']);}catch(Throwable){return Response::text("Anexo indisponível.\n",404);}},$viewWhatsApp);
