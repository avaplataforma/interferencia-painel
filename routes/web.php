...
return static function (
    ...
    $router->get('/external-forms',static function()use($view,$externalForms,$session,$basePath,$browserTitle,$config):Response{return $view->render('external-forms/index',['title'=>'Formulários externos — '.$browserTitle,'forms'=>$externalForms->all(),'appUrl'=>rtrim($config->string('app.url'),'/'),'message'=>$session->get('external_forms.message'),'error'=>$session->get('external_forms.error'),'basePath'=>$basePath]);},$manageExternalForms);
    $externalFormView=static function(?int $id=null)use($view,$externalForms,$tags,$contacts,$session,$csrf,$basePath,$browserTitle):Response{$form=$id===null?null:$externalForms->find($id);if($id!==null&&$form===null)return Response::text("Formulário não encontrado.\n",404);return $view->render('external-forms/form',['title'=>($id===null?'Novo formulário':'Editar formulário').' — '.$browserTitle,'formItem'=>$form,'tags'=>$tags->all(true),'statuses'=>$contacts->statuses(),'error'=>$session->get('external_forms.error'),'csrfField'=>$csrf->field(),'basePath'=>$basePath]);};
