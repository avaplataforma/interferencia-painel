...
return static function (
    ...
    $router->post('/finance/sync',static function()use($asaasSynchronizer,$session,$basePath):Response{try{$result=$asaasSynchronizer->syncBatch();$detail=$result['customers']>0?sprintf('%d cliente(s) importado(s).',$result['customers']):sprintf('%d cobrança(s) importada(s).',$result['payments']);$session->flash('finance.message',$result['complete']?'Sincronização concluída. '.$detail:$detail.' Clique novamente para continuar do ponto salvo.');}catch(Throwable$e){$session->flash('finance.error',$e->getMessage());}return Response::redirect($basePath.'/finance');},[$requireAuth,new RequirePermission($auth,'finance.manage')]);
    ...
    $router->get('/finance/customers',static function(Request$request)use($view,$finance,$financeScope,$auth,$session,$basePath,$browserTitle):Response{$search=trim((string)($request->queryValue('q','')??''));$scope=(string)($request->queryValue('scope','all')??'all');if(!in_array($scope,['all','legacy','units'],true))$scope='all';if(!$auth->can('finance.legacy_view')&&$scope==='legacy')$scope='units';$page=max(1,(int)($request->queryValue('page','1')??'1'));return$view->render('finance/customers/index',['title'=>'Clientes financeiros — '.$browserTitle,'result'=>$finance->customers($financeScope(),$auth->can('finance.legacy_view'),$search,$scope,$page),'search'=>$search,'scope'=>$scope,'canViewLegacy'=>$auth->can('finance.legacy_view'),'message'=>$session->get('finance_customers.message'),'error'=>$session->get('finance_customers.error'),'basePath'=>$basePath]);},$viewFinance);
