...
return static function (
    ...
    $router->get('/whatsapp',static function(Request$request)use($view,$whatsappLines,$whatsappMessages,$auth,$csrf,$session,$basePath,$browserTitle):Response{$lines=$whatsappLines->authorizedForUser($auth->user()->id);$lineIds=array_map(static fn(array$l):int=>(int)$l['id'],$lines);$scope=(string)($request->queryValue('scope','all')??'all');if(!in_array($scope,['all','mine','unassigned','unread'],true))$scope='all';$search=trim((string)($request->queryValue('q','')??''));$conversationId=(int)($request->queryValue('conversation','0')??'0');$conversations=$whatsappMessages->conversations($lineIds,$scope,$search,$auth->user()->id);$active=$conversationId>0?$whatsappMessages->conversation($conversationId,$lineIds):($conversations[0]??null);$messages=[];$linkable=[];if($active!==null){if(isset($active['last_body']))$active=$whatsappMessages->conversation((int)$active['id'],$lineIds);if($active!==null){$messages=$whatsappMessages->messages((int)$active['id']);$whatsappMessages->markRead((int)$active['id']);$linkable=$whatsappMessages->linkableContacts((int)$active['id'],$lineIds);}}return$view->render('whatsapp/inbox',['title'=>'WhatsApp — '.$browserTitle,'lines'=>$lines,'conversations'=>$conversations,'activeConversation'=>$active,'messages'=>$messages,'attendants'=>$whatsappMessages->attendants($lineIds),'linkableContacts'=>$linkable,'scope'=>$scope,'search'=>$search,'demoMode'=>$conversations===[],'message'=>$session->get('whatsapp.message'),'error'=>$session->get('whatsapp.error'),'csrfField'=>$csrf->field(),'basePath'=>$basePath]);},$viewWhatsApp);
    $router->post('/whatsapp/{id:\d+}/assign',static function(Request$request,array$params)use($whatsappLines,$whatsappMessages,$auth,$session,$basePath):Response{$lineIds=array_map(static fn(array$l):int=>(int)$l['id'],$whatsappLines->authorizedForUser($auth->user()->id));$userId=(int)$request->input('user_id',$auth->user()->id);$ok=$whatsappMessages->assign((int)$params['id'],$userId,$lineIds);$session->flash($ok?'whatsapp.message':'whatsapp.error',$ok?'Conversa atribuída.':'Não foi possível atribuir a conversa.');return Response::redirect($basePath.'/whatsapp?conversation='.(int)$params['id']);},$viewWhatsApp);
