...
return static function (
    ...
    $saveContact=static function(Request $request,?int $id=null) use($validator,$contactManager,$units,$contactUnit,$auth,$session,$basePath):Response {
        ...
        try { if($result->fails())throw new RuntimeException(implode(' ',array_map(static fn(array $errors):string=>$errors[0],$result->errors()))); $allowedPoles=array_map(static fn(array $item):string=>(string)$item['name'],array_filter($units->all(),static fn(array $item):bool=>(int)$item['is_active']===1));if(!in_array((string)$result->value('origin_city'),$allowedPoles,true))throw new RuntimeException('Selecione um Polo/Cidade válido.'); $data=$request->inputData(); foreach($result->values() as $key=>$value)$data[$key]=$value; $savedId=$contactManager->save($id,(int)$unit['id'],$auth->user()->id,$data); $session->flash('contacts.message',$id===null?'Contato criado.':'Contato atualizado.'); return Response::redirect($basePath.'/crm/contacts/'.$savedId); }
    ...
    $router->get('/whatsapp',static function(Request$request)use($view,$whatsappLines,$whatsappMessages,$auth,$basePath,$browserTitle):Response{$lines=$whatsappLines->authorizedForUser($auth->user()->id);$lineIds=array_map(static fn(array$l):int=>(int)$l['id'],$lines);$scope=(string)($request->queryValue('scope','all')??'all');if(!in_array($scope,['all','mine','unassigned','unread'],true))$scope='all';$search=trim((string)($request->queryValue('q','')??''));$conversationId=(int)($request->queryValue('conversation','0')??'0');$conversations=$whatsappMessages->conversations($lineIds,$scope,$search,$auth->user()->id);$active=$conversationId>0?$whatsappMessages->conversation($conversationId,$lineIds):($conversations[0]??null);$messages=[];if($active!==null){if(isset($active['last_body']))$active=$whatsappMessages->conversation((int)$active['id'],$lineIds);if($active!==null){$messages=$whatsappMessages->messages((int)$active['id']);$whatsappMessages->markRead((int)$active['id']);}}return$view->render('whatsapp/inbox',['title'=>'WhatsApp — '.$browserTitle,'lines'=>$lines,'conversations'=>$conversations,'activeConversation'=>$active,'messages'=>$messages,'scope'=>$scope,'search'=>$search,'demoMode'=>$conversations===[],'basePath'=>$basePath]);},$viewWhatsApp);
