...
return static function (
    ...
    $router->get('/whatsapp',static function(Request$request)use($view,$whatsappLines,$whatsappMessages,$whatsappTemplates,$whatsappCloudApi,$auth,$csrf,$session,$basePath,$browserTitle):Response{$lines=$whatsappLines->authorizedForUser($auth->user()->id);$lineIds=array_map(static fn(array$l):int=>(int)$l['id'],$lines);$scope=(string)($request->queryValue('scope','open')??'open');if(!in_array($scope,['all','open','mine','unassigned','unread','overdue','closed'],true))$scope='open';$search=trim((string)($request->queryValue('q','')??''));$conversationId=(int)($request->queryValue('conversation','0')??'0');$conversations=$whatsappMessages->conversations($lineIds,$scope,$search,$auth->user()->id);$active=$conversationId>0?$whatsappMessages->conversation($conversationId,$lineIds):($conversations[0]??null);$messages=[];$linkable=[];$sendAvailability=['allowed'=>false,'reason'=>'Selecione uma conversa.'];if($active!==null){if(isset($active['last_body']))$active=$whatsappMessages->conversation((int)$active['id'],$lineIds);if($active!==null){$messages=$whatsappMessages->messages((int)$active['id']);$whatsappMessages->markRead((int)$active['id']);$linkable=$whatsappMessages->linkableContacts((int)$active['id'],$lineIds);$sendAvailability=$whatsappMessages->sendAvailability((int)$active['id'],$lineIds,$auth->user()->id,$whatsappCloudApi->ready());}}$hasAny=$active!==null||$conversations!==[]||$whatsappMessages->conversations($lineIds,'all','',$auth->user()->id)!==[];return$view->render('whatsapp/inbox',['title'=>'WhatsApp — '.$browserTitle,'lines'=>$lines,'conversations'=>$conversations,'activeConversation'=>$active,'messages'=>$messages,'attendants'=>$whatsappMessages->attendants($lineIds),'linkableContacts'=>$linkable,'templates'=>$whatsappTemplates->active(),'scope'=>$scope,'search'=>$search,'sendAvailability'=>$sendAvailability,'demoMode'=>!$hasAny,'message'=>$session->get('whatsapp.message'),'error'=>$session->get('whatsapp.error'),'csrfField'=>$csrf->field(),'basePath'=>$basePath]);},$viewWhatsApp);
    ...
    $router->post('/whatsapp/simulator',static function(Request$request)use($validator,$whatsappLines,$whatsappMessages,$session,$basePath):Response{$result=$validator->validate($request->inputData(),['line_id'=>'required|string','name'=>'required|string|min:2|max:160','phone'=>'required|string|max:20','message'=>'required|string|min:1|max:4096'],['line_id'=>'linha','name'=>'nome','phone'=>'telefone','message'=>'mensagem']);try{if($result->fails())throw new RuntimeException(implode(' ',array_map(static fn(array$errors):string=>$errors[0],$result->errors())));$lineId=(int)$result->value('line_id');$line=$whatsappLines->find($lineId);if($line===null||(int)$line['is_active']!==1)throw new RuntimeException('Selecione uma linha ativa.');$conversation=$whatsappMessages->simulateInbound($lineId,(string)$result->value('name'),(string)$result->value('phone'),(string)$result->value('message'));$session->flash('whatsapp.message','Mensagem de teste recebida.');return Response::redirect($basePath.'/whatsapp?conversation='.$conversation);}catch(Throwable$e){$session->flash('whatsapp.simulator.error',$e->getMessage());return Response::redirect($basePath.'/whatsapp/simulator');}},$simulateWhatsApp);
    ...
    $templateForm=static function(?int$id=null)use($view,$whatsappTemplates,$session,$csrf,$basePath,$browserTitle):Response{$template=$id===null?null:$whatsappTemplates->find($id);if($id!==null&&$template===null)return Response::text("Modelo não encontrado.\n",404);return$view->render('whatsapp/templates/form',['title'=>($id===null?'Novo modelo':'Editar modelo').' — '.$browserTitle,'template'=>$template,'error'=>$session->get('whatsapp_templates.error'),'csrfField'=>$csrf->field(),'basePath'=>$basePath]);};
